...
<div id="featured-slider" class="<?php echo esc_attr(ashe_options( 'general_slider_width' )) === 'boxed' ? 'boxed-wrapper': ''; ?>" data-slick="<?php echo esc_attr( $slider_data ); ?>">
	...
	while ( $sliderQuery->have_posts() ) : $sliderQuery->the_post();
		...
		if ( ashe_is_preview() ) {
			...
			$preview_count++;
			 ...
			 // modif : définir la tille d'affichage de l'image en large, medium et thumbnail selon la taille de l'ecran
			$featured_image_large = get_the_post_thumbnail_url(null, 'large');
			$featured_image_medium = get_the_post_thumbnail_url(null, 'medium');
			$featured_image_thumbnail = get_the_post_thumbnail_url(null, 'thumbnail');
		}
	...
	<div class="slider-item">
		...
		<div class="slider-item-bg">
			<!-- modif : définir la tille d'affichage de l'image en large, medium et thumbnail dans srcset et sizes -->
			<!-- modif : ajouter fetchpriority="high" pour réduire tout délai inutile de chargement des images -->
			<img 
			fetchpriority="high"
			src="<?php echo esc_url($featured_image_large); ?>"
			srcset="<?php echo esc_url($featured_image_large); ?> 1080w,
					<?php echo esc_url($featured_image_medium); ?> 720w,
					<?php echo esc_url($featured_image_thumbnail); ?> 360w"
			sizes="(max-width: 600px) 100vw, (max-width: 1024px) 50vw, 1080px"
